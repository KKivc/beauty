...
function testHighBoostProfileKeepsLowHalfAndStrengthensHighEnd(testCase)
half = beautySmoothingProfile(50);
verifyEqual(testCase, half.highBoost, 0, 'AbsTol', 1e-12);
verifyEqual(testCase, half.baseWeight, .16 + .38 * .5 ^ .85, ...
    'AbsTol', 1e-12);
verifyEqual(testCase, half.fineRetention, 1 - .36 * sqrt(.5) + ...
    .10 * .5 - .04 * .5 ^ 2, ...
    'AbsTol', 1e-12);

maximum = beautySmoothingProfile(100);
highNatural = beautySmoothingProfile(75);
verifyEqual(testCase, maximum.highBoost, 1, 'AbsTol', 1e-12);
verifyGreaterThan(testCase, maximum.highBoost, highNatural.highBoost);
verifyEqual(testCase, maximum.baseWeight, highNatural.baseWeight, ...
    'AbsTol', 1e-12);
...
verifyEqual(testCase, maximum.mediumRetention, highNatural.mediumRetention, ...
    'AbsTol', 1e-12);
verifyEqual(testCase, maximum.chromaEffect, highNatural.chromaEffect, ...
    'AbsTol', 1e-12);
end
...
context = struct('skinMask', ones(imageSize), ...
    ...
    'featureProtectionMask', zeros(imageSize), ...
...
[~, hard, diagnostics] = buildFeatureProtectionMasks( ...
    image, parsing.regions, parsing.regionConfidence, 60);
...
[~, hard, diagnostics] = buildFeatureProtectionMasks( ...
    image, parsing.regions, parsing.regionConfidence, 80);
...
[protection, hard, diagnostics] = buildFeatureProtectionMasks( ...
    image, parsing.regions, parsing.regionConfidence, 80);
