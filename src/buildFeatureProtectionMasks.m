function [protection, hard, diagnostics] = buildFeatureProtectionMasks( ...
        inputImage, regions, confidence, faceScale)
%BUILDFEATUREPROTECTIONMASKS 分类别保护眼眉、鼻部结构和嘴唇边界。
if ~isa(inputImage, 'uint8') || ndims(inputImage) ~= 3 || ...
        size(inputImage, 3) ~= 3 || ~isscalar(faceScale) || faceScale <= 0
    error('buildFeatureProtectionMasks:InvalidInput', ...
        'A uint8 RGB image and positive face scale are required.');
end
...
occluderRadius = min(6, max(2, round(.012 * faceScale)));
...
hard = occluderHard | lipCore | nostrilCore | lashCore;
protection = max(cat(3, occluderProtection, browProtection, ...
    lipProtection, noseProtection, eyeDetailProtection), [], 3);
protection = double(min(max(protection, 0), 1));
...
for index = 1:numel(names)
    ...
    if ~isfield(regions, name) || ~isfield(confidence, name)
        error('buildFeatureProtectionMasks:InvalidRegions', ...
            'All requested face classes must be present.');
    end
