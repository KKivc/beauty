...
%   同值；Mid 兼容门控、strength、频段分解与合成公式不变。Fine alpha
%   与输出增量相对 legacy 仅有 1-x 补码与乘法结合顺序的浮点噪声
%   （≤1e-15，T07 推导基线 1.11e-16 同量级）。
[image, faceBox, parsing] = fixtureImage(120, 160);
context = prepareBeautyContext(image, faceBox, parsing, ...
    emptyBodyParsing([120, 160]));
[beautyMasks, ~] = masks.buildBeautyMasks(image, context, faceBox);
...
for index = 1:numel(strengths)
    ...
    % 统计标量与合并保护图 bit-exact 不变；Mid 兼容门控逐字段不变。
    ...
    verifyEqual(testCase, stageDetails.noseMidGate, ...
        legacyDetails.noseMidGate, 'AbsTol', 0);
    verifyLessThanOrEqual(testCase, ...
