...
%       保留 >=5% Fine 处理量）、smoothingMid=.90、regionBandBase=.95
%       （baseLuminance 分级保护，T32 起不再折进 target.baseLuminance，
%       改经 T30 纯 policy 带注入逐像素 supportMap）；
...
% baseLuminance：T32 起耳结构带的亮度保护经纯 policy 带 regionBandBase
% 承载（消费侧 regionBandGate = 1 - regionBandBase 注入逐像素 supportMap），
% 不再折进 target.baseLuminance。带内 regionBandBase = .95（与 policyTexture
% 通道同档；.80 是 max 下界，与 T21 鼻结构带同一写法）。
verifyGreaterThanOrEqual(testCase, ...
    policy.regionBandBase(corePoint(1), corePoint(2)), .80 - 1e-12, ...
    'ear 结构带必须给出 >= .80 的亮度结构保护下界。');
verifyEqual(testCase, ...
    policy.regionBandBase(corePoint(1), corePoint(2)), .95, 'AbsTol', 1e-12);
verifyEqual(testCase, policy.target.tone(corePoint(1), corePoint(2)), ...
    legacy.target.tone(corePoint(1), corePoint(2)), 'AbsTol', 0, ...
    '耳部无 identity 色度语义，tone 不得被耳带抬升。');
verifyEqual(testCase, policy.target.whitening(corePoint(1), corePoint(2)), ...
    legacy.target.whitening(corePoint(1), corePoint(2)), 'AbsTol', 0, ...
...
verifyGreaterThanOrEqual(testCase, ...
    min(policy.regionBandBase(:) - legacy.regionBandBase(:)), 0);
% T32：耳部 evidence 不改变逐像素修改门 target.baseLuminance（该字段只
% 承载 legacy general 折叠），耳带保护只经 regionBandBase 注入。
verifyEqual(testCase, policy.target.baseLuminance, ...
    legacy.target.baseLuminance, 'AbsTol', 0, ...
    'T32 起耳带保护不再折进 target.baseLuminance。');
verifyEqual(testCase, policy.target.tone, legacy.target.tone, 'AbsTol', 0, ...
    'T22 不得给 tone 增加耳部项（肤色与脸颊/颈部连续）。');
verifyEqual(testCase, policy.target.whitening, legacy.target.whitening, 'AbsTol', 0, ...
...
verifyEqual(testCase, protection.target.tone, earOnlyProtection.target.tone, 'AbsTol', 0, ...
    'T22 不得给 tone 增加耳部项（耳-颊肤色连续）。');
verifyEqual(testCase, protection.target.whitening, ...
    earOnlyProtection.target.whitening, 'AbsTol', 0, ...
...
verifyEqual(testCase, ...
    mean(protection.target.tone(earRegion)) - mean(protection.target.tone(cheekRegion)), ...
    mean(legacyProtection.target.tone(earRegion)) - ...
    mean(legacyProtection.target.tone(cheekRegion)), 'AbsTol', 1e-12, ...
    '耳-颊 tone 保护差必须与 legacy 相同（不新增割裂）。');
verifyEqual(testCase, ...
    mean(protection.target.whitening(earRegion)) - ...
    mean(protection.target.whitening(cheekRegion)), ...
    mean(legacyProtection.target.whitening(earRegion)) - ...
    mean(legacyProtection.target.whitening(cheekRegion)), 'AbsTol', 1e-12, ...
    '耳-颊 whitening 保护差必须与 legacy 相同（不引入异色块）。');

% 结构保护：耳结构带内 Fine 严格抬升，Mid 与 regionBandBase 只增不减。
...
verifyGreaterThanOrEqual(testCase, ...
    min(protection.regionBandBase(structureBand) - ...
    earOnlyProtection.regionBandBase(structureBand)), 0);
