...
%BUILDBEAUTYMASKS 聚合 v3.1 的保护 Mask、硬保护和连续强度图。
...
hasWhitening = isfield(beautyContext, 'whiteningProtectionMask');
refreshChromaProtection = ~hasChromaProtectionMask || ~hasWhitening;
...
if ~hasWhitening || needsDerivedDiagnostics
    [generatedWhitening, whiteningDiagnostics] = ...
        masks.buildWhiteningProtectionMask(inputImage, beautyContext, ...
        faceBox, textureDiagnostics);
else
    generatedWhitening = [];
    whiteningDiagnostics = struct('reusedDerivedMask', true);
end
if refreshChromaProtection || needsDerivedDiagnostics
    [generatedChroma, chromaDiagnostics] = ...
        masks.buildChromaProtectionMask(inputImage, beautyContext, faceBox);
else
    generatedChroma = [];
    chromaDiagnostics = struct('reusedDerivedMask', true);
end
...
if hasWhitening
    whiteningProtectionMask = readMask(beautyContext, ...
        'whiteningProtectionMask', imageSize);
else
    whiteningProtectionMask = generatedWhitening;
end
if hasChromaProtectionMask && ~refreshChromaProtection
    toneProtectionMask = chromaProtectionMask;
else
    toneProtectionMask = generatedChroma;
...
diagnostics = struct( ...
    ...
    'chroma', chromaDiagnostics, ...
    'tone', chromaDiagnostics, ...
