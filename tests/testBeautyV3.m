...
% T31/T32：发布规范双门控 target.*/support.*（+ 独立 hard、过渡扁平字段
% noseMidProtection/tone/whitening/regionBand*）。T07 的扁平折叠名
% smoothingFine/smoothingMid/repairFine/repairMid/baseLuminance 已被
...
verifyFalse(testCase, isfield(protection, 'baseLuminance'), ...
    'T32 起旧扁平折叠名 baseLuminance 必须删除。');
...
for index = 1:numel(strengths)
    ...
    verifyTrue(testCase, isequaln(whiteStage, ...
        rmfield(whiteLegacy, 'whiteningProtectionMask')), ...
...
verifyTrue(testCase, isfield(legacyDetails60, ...
    ...
    ~isfield(legacyDetails60, 'whiteningGateSnapshot'));
verifyEqual(testCase, legacyDetails60.whiteningProtectionMask, ...
    beautyMasks.whiteningProtectionMask, 'AbsTol', 0);
verifyEqual(testCase, isfield(whiteStage, ...
    'whiteningProtectionMask'), false, ...
    'stage 结果不得再报告 whitening 兼容 alias 字段。');
verifyTrue(testCase, isfield(whiteLegacy, 'whiteningProtectionMask'), ...
    'legacy 结果字段保持不变。');
...
function contract = makeTestWhiteningContract(beautyMasks, protection)
%MAKETESTWHITENINGCONTRACT 复现生产端 makeWhiteningStageContract（T18）
%   的未折叠字段公式（whitening/faceSkin 来源与 consumer legacy 路径
%   同源，含缺省回退），供消费侧等价断言与扰动负向证明使用。
...
structureGate(faceSkinSupport) = 1 - .10 * ...
    structureProtection(faceSkinSupport);
whiteningProtection = double(beautyMasks.whiteningProtectionMask);
contract = struct( ...
    'whitening', protection.whitening, ...
    'hard', protection.hard, ...
