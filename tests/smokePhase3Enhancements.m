...
for pairIndex = 1:size(pairs, 1)
    ...
    if ~hasFace
        error('smokePhase3Enhancements:NoFace', ...
            '第 %d 个私有 smoke 图像未检测到语义人脸。', pairIndex);
    end
    context = normalizeBeautyContext(previewImage, faceBox, ...
        prepareBeautyContext(previewImage, faceBox, ...
    ...
    evaluationMask = imerode(context.faceSkinMask > .5, ...
        strel('disk', 2, 0)) & beautyMasks.protectionMask < .2;
    ...
    visualName = sprintf('private-%03d_phase3.jpg', pairIndex);
    ...
    rows(pairIndex).image = sprintf('private-%03d', pairIndex);
    ...
    rows(pairIndex).visualPath = visualName;
    if previewScale <= 0
...
% Base-only 亮度均匀化允许小幅改变全局 Y 中位数；组合验收另行对
% 低提亮选区使用 2/255 绝对差门槛。这里仅拦截明显的整体偏色。
if any(summary.maximumChannelMedianShift > 4 / 255)
    error('smokePhase3Enhancements:ToneShift', ...
        '磨皮改变了皮肤整体色调：最大通道中位偏移超过 4/255。');
end
end
...
context = normalizeBeautyContext(preview, faceBox, ...
    prepareBeautyContext(preview, faceBox, ...
    detection.selectedParsing, struct('rotationDegrees', ...
    detection.orientationDegrees)));
[beautyMasks, ~] = masks.buildBeautyMasks(preview, context, faceBox);
...
noseHardFraction = nnz(noseInterior & ...
    beautyMasks.hardProtectionMask >= .999) / nnz(noseInterior);
activeNose = noseInterior & beautyMasks.protectionMask < .80;
...
visualName = 'private-17_orientation_phase3.jpg';
visualFile = fullfile(outputFolder, visualName);
featureVisual = repmat(uint8(round(255 * ...
    beautyMasks.protectionMask)), [1, 1, 3]);
imwrite([preview, output, featureVisual], visualFile, 'Quality', 94);
visualPath = visualName;
...
context = normalizeBeautyContext(source, faceBox, ...
    prepareBeautyContext(source, faceBox, detection.selectedParsing, ...
    struct('probabilities', probabilities)));
[beautyMasks, ~] = masks.buildBeautyMasks(source, context, faceBox);
...
whiteningOutput = beautifyImage(source, struct( ...
    'smoothingStrength', 0, 'whiteningStrength', 15), faceBox, context);
difference = max(abs(double(output) - double(source)), [], 3);
backgroundChange = max(difference(context.skinMask < .01), [], 'all');
hardChange = max(difference(beautyMasks.hardProtectionMask >= .999), [], 'all');
...
faceEvaluation = imerode(context.faceSkinMask > .50, ...
    strel('disk', 2, 0)) & beautyMasks.protectionMask < .35;
...
% 脸外弱化只测量独立美白增量；组合结果中的磨皮会降低脸部
% 低频亮度，直接用组合 Y 计算该比值会把身体相对亮度虚高。
faceWhiteningIncrease = median(whiteningYCbCr(faceEvaluation) - ...
    sourceYCbCr(faceEvaluation));
bodyWhiteningIncrease = median(whiteningYCbCr(bodyEvaluation) - ...
    sourceYCbCr(bodyEvaluation));
...
if bodyToFaceChange > .95 || ...
        bodyWhiteningIncrease > .55 * max(faceWhiteningIncrease, eps) || ...
...
visualName = 'private-80_arm_phase3.jpg';
visualFile = fullfile(outputFolder, visualName);
maskVisual = repmat(uint8(round(255 * context.skinMask)), [1, 1, 3]);
imwrite([source, output, combinedOutput, maskVisual], visualFile, 'Quality', 94);
visualPath = visualName;
result = struct('rightArmRecall', recall, ...
    ...
    'faceWhiteningIncrease', faceWhiteningIncrease, ...
    'bodyWhiteningIncrease', bodyWhiteningIncrease, ...
