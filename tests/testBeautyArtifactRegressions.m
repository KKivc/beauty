...
verifyGreaterThan(testCase, mean(actualFineRetention(evaluation)), .40, ...
...
verifyEqual(testCase, protection.target.tone, legacyProtection.target.tone, ...
    'AbsTol', 0, 'T21 不得给 tone 增加鼻部项。');
diffMap = mean(abs(double(policyOut) - double(legacyOut)), 3);
% V4 严格 Repair 由非零 region policy 启用，并在整个可处理皮肤域把
% 瑕疵修复收敛为紧凑 Fine-first 路径；与 compat 输出的差异因此允许
% 出现在皮肤域，但不得泄漏到非皮肤区域。
inBandUnion = nostrilBand > 0 | structureBand > 0 | ...
    beautyMasks.skinMask > .01;
...
verifyGreaterThanOrEqual(testCase, min(protection.target.whitening(softBand)), ...
...
verifyEqual(testCase, protection.target.tone, earOnlyProtection.target.tone, ...
...
diffMap = mean(abs(double(policyOut) - double(legacyOut)), 3);
% 非零 eye/lip policy 同时启用 V4 严格 Repair；最终输出相对 compat
% 的变化允许落在可处理皮肤域，非皮肤区域仍必须逐位一致。
