...
%     非法 evidence（尺寸/取值/类型）fail-fast。
    ...
    verifyEqual(testCase, policyZero.target.smoothingFine(eyePoint(1), eyePoint(2)), ...
        .95, 'AbsTol', 1e-12);
    verifyEqual(testCase, policyZero.target.smoothingMid(eyePoint(1), eyePoint(2)), ...
        .90, 'AbsTol', 1e-12);
    verifyEqual(testCase, policyZero.target.repairFine(eyePoint(1), eyePoint(2)), ...
        .95, 'AbsTol', 1e-12);
    ...
    verifyEqual(testCase, policyZero.target.whitening(eyePoint(1), eyePoint(2)), ...
        .85, 'AbsTol', 1e-12);
    verifyEqual(testCase, policyZero.target.tone(eyePoint(1), eyePoint(2)), 0, ...
        'AbsTol', 1e-12, '眼周色度无 identity 语义，tone 不得被眼带抬升。');
    lipPoint = fixture.lipDetailPoint;
    verifyEqual(testCase, policyZero.target.tone(lipPoint(1), lipPoint(2)), .90, ...
    ...
    verifyEqual(testCase, ...
        policyFull.target.smoothingFine(transitionPoint(1), transitionPoint(2)), ...
        expectedCap, 'AbsTol', 1e-12);
    verifyEqual(testCase, ...
        legacyFull.target.smoothingFine(transitionPoint(1), transitionPoint(2)), 1, ...
    ...
    verifyTrue(testCase, ...
        all(policyFull.target.smoothingFine(activeTransition) <= ...
    ...
    % 带外（evidence 全零像素）逐位还原 legacy。T31 起 protection 为
    % target.*/support.* 双门控嵌套，先展平为叶子字段再逐像素比较。
    outside = evidence.periocular == 0 & evidence.lip == 0;
    flatPolicy = flattenProtection(policyFull);
    flatLegacy = flattenProtection(legacyFull);
    fieldNames = fieldnames(flatPolicy);
    for fieldIndex = 1:numel(fieldNames)
        fieldName = fieldNames{fieldIndex};
        verifyEqual(testCase, ...
            flatPolicy.(fieldName)(outside), flatLegacy.(fieldName)(outside), ...
...
verifyEqual(testCase, protection.target.smoothingFine(outside), ...
    legacyProtection.target.smoothingFine(outside), 'AbsTol', 0, ...
...
whiteningLegacyOut = beautifyImage(sourceImage, whiteningParams, faceBox, ...
    legacyContext);
whiteningSupport = protection.regionBandWhitening > 0 | ...
    bareContext.evidence.colorSensitiveSkin > 0;
outsideWhitening = ~whiteningSupport;
...
noWhiteningBandContext.evidence = zeroEvidenceFields( ...
    noWhiteningBandContext.evidence, ...
    {'periocular', 'lip', 'nostril', 'colorSensitiveSkin'}, ...
...
verifyGreaterThanOrEqual(testCase, ...
    min(protection.target.whitening(softBand)), ...
