...
%
%   T33（执行契约）：Tone/Whitening 与 T31/T32 同构发布规范双门控
%   target.tone/support.tone、target.whitening/support.whitening，并把
%   T07 扁平折叠名 tone/whitening 删除（同一语义只留一个规范字段）；
%   同时把 tone/whitening 执行层的未折叠门控（structureGate/featureGate/
%   uniformFeatureGate、structureGate/featureGate）与鼻部幅度上限上移到
%   本层发布（toneGates/whiteningGates/whiteningAmplitudeCeiling），执行层
%   不再读 legacy general mask 重建门控。零带时四个规范字段逐位等于对应
%   legacy 折叠值 / pre-T30 参考门。
%
...
% 幅度上限标量。无鼻部结构时发布 Inf（min(x,Inf)=x，逐位还原生产原式）；
% 有鼻部结构时发布 .07（与旧分支字面量一致，逐位等价）。该上限是"鼻梁/
% 鼻侧结构在 RGB 裁切前保留"的退让语义，属 protection 决策而非执行层分支。
if any(nose(:) > .5)
    whiteningAmplitudeCeiling = .07;
else
    whiteningAmplitudeCeiling = Inf;
end
...
% （T31/T32）；T33 同样把扁平折叠名 tone/whitening 迁入 target.* 并删除。
protection = struct( ...
    'hard', hard, ...
    'target', struct( ...
    ...
    'tone', clamp01(toneSupportProtection), ...
    'whitening', clamp01(whiteningSupportProtection)), ...
    'noseMidProtection', clamp01(.50 * nose), ...
    'toneGates', toneGates, ...
    'whiteningGates', whiteningGates, ...
    'whiteningAmplitudeCeiling', whiteningAmplitudeCeiling, ...
