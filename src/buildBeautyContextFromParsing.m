...
%BUILDBEAUTYCONTEXTFROMPARSING 从 19 类解析概率构建 V4 分层 Context。
%   T08 起生产链默认输出 schemaVersion='4.0' 的 canonical 分层结构：
%     semantic/processability — T05 分层（本函数末尾构建）；
%     evidence / diagnostics.policyEvidence — T06 分层与元数据；
%     protection — T07 stage 分层；
%   三者均由权威桥接 rebuildBeautyDerivedMasks 统一生成。skinMask、
%   textureProtectionMask 等顶层字段是迁移期 compat alias：仅为旧
%   consumer 与运行时缓存指纹保留，新代码不得新增依赖。
