...
% T14：生产管线（beautifyImage）把生产端拼装的 repair-fine stage
%   contract 传给 repairSkinBlemishes。注入瑕疵 + 结构保护场景下，生产
%   stage 路径的 fineWeight/fineCorrection 与 legacy 直接重算逐位等价
%   （bit-exact，v3.2 texture linear gate 完整保留），共享
%   referenceReliability/mediumWeight/chromaWeight 与独立 stage 重算的
%   fineGateRuntime bit-exact 一致；cached/uncached 两条路径的最终 RGB
%   与 repair 诊断完全一致（缓存复用不得改变 contract 的组装与消费）。
imageSize = [160, 200];
[yGrid, xGrid] = ndgrid(1:imageSize(1), 1:imageSize(2));
rng(11);
luma = 0.55 + 0.02 * sin(2 * pi * xGrid / 23) .* ...
...
contract = makeRegressionRepairFineContract(beautyMasks, protection, ...
    blemishMap);
...
function contract = makeRegressionRepairFineContract(beautyMasks, ...
    protection, blemishMap)
%MAKEREGRESSIONREPAIRFINECONTRACT 复现生产端
%   makeRepairFineStageContract（T14）的未折叠字段公式。
