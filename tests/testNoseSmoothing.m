...
verifyEqual(testCase, ...
    policy.target.whitening(corePoint(1), corePoint(2)), .85, 'AbsTol', 1e-12);
...
verifyEqual(testCase, ...
    policy.target.whitening(ridgePoint(1), ridgePoint(2)), ...
    legacy.target.whitening(ridgePoint(1), ridgePoint(2)), 'AbsTol', 0, ...
...
context = buildBeautyContextFromParsing(image, faceBox, parsing);
bareContext = context;
bareContext.evidence = zeroEvidenceFields(bareContext.evidence, ...
    {'colorSensitiveSkin'}, size(image, 1:2));
...
verifyEqual(testCase, protection.target.tone, legacyProtection.target.tone, 'AbsTol', 0, ...
