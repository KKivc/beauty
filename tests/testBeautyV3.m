...
%   权重、chromaWeight 与共享 referenceReliability 保持 legacy 计算
%   （Mid 未迁移，bit-exact 不变）。
[image, faceBox, parsing] = fixtureImage(120, 160);
context = prepareBeautyContext(image, faceBox, parsing, ...
    emptyBodyParsing([120, 160]));
[beautyMasks, ~] = masks.buildBeautyMasks(image, context, faceBox);
...
for caseIndex = 1:numel(blemishCases)
    ...
    contract = makeTestRepairFineContract(beautyMasks, protection, ...
        blemishMap);
    for index = 1:numel(strengths)
        strength = strengths(index);
        ...
        % 共享路径 bit-exact：Mid 未迁移，referenceReliability/chroma
        % 与 structureGate 保持 legacy 计算。
...
% fineCorrection 不在此断言：Mid 未迁移时 referenceReliability 是共享
% legacy 路径（仍读取 textureProtectionMask），texture 扰动经共享参考
% 传播到 fineCorrection 属预期行为；Fine 权重本身的 texture 独立性由
% 上一断言证明。
...
function contract = makeTestRepairFineContract(beautyMasks, protection, ...
    blemishMap)
%MAKETESTREPAIRFINECONTRACT 复现生产端 makeRepairFineStageContract
%   （T14）的未折叠字段公式，供消费侧重建断言与扰动负向证明使用。
